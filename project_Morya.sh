...
cat allsubs1.txt | anew | tee allsubs1.txt
wc -l allsubs1.txt | awk '{print $1 " subdomains founded by Subfinder"}' | notify --silent

printf "Running CRT.sh\n" | notify --silent
python3 $HOME/tools/ctfr/ctfr.py -d $DOMAIN -o allsubs2.txt
cat allsubs2.txt | anew | tee allsubs2.txt
...
cat allsubs4.txt | anew | tee allsubs4.txt
...
prinf "Runnig Gauplus\n" | notify --silent
gauplus $DOMAIN | unfurl -u domains | anew allsubs10.txt
wc -l allsubs10.txt | awk '{print $1 " subdomains founded by Gauplus"}' | notify --silent

printf "Running Github-subdomains\n" | notify --silent
github-subdomains -d $DOMAIN -t $HOME/.config/github-subdomains/tokens.txt -o allsubs11.txt
cat allsubs11.txt | anew | tee allsubs11.txt
...
wc -l allsubs13.txt | awk '{print $1 " subdomains founded by tls.bufferover"}' | notify --silent

printf "Running dns.bufferover\n" | notify --silent
curl "https://dns.bufferover.run/dns?q=$DOMAIN" | jq -r .FDNS_A'[]',.RDNS'[]' | cut -d ',' -f2 | grep -F "$DOMAIN" | anew allsubs14.txt
wc -l allsubs14.txt | awk '{print $1 " subdomains founded by dns.bufferover"}' | notify --silent

printf "Running Puredns\n" | notify --silent
dnsvalidator -tL https://public-dns.info/nameservers.txt -threads 500 -o resolvers.txt
puredns bruteforce $HOME/wordlist/best-dns-wordlist.txt $DOMAIN -r resolvers.txt -w allsubs15.txt --wildcard-batch 1000000
cat allsubs15.txt | anew allsubs15.txt
wc -l allsubs15.txt | awk '{print $1 " subdomains founded by Puredns"}' | notify --silent
...
dnsvalidator -tL https://public-dns.info/nameservers.txt -threads 500 -o resolvers.txt
puredns resolve permutations.txt -r resolvers.txt --wildcard-batch 1000000 -w allsubs16.txt
cat allsubs16.txt | grep "\.${DOMAIN}" | anew allsubs16.txt
wc -l allsubs16.txt | awk '{print $1 " subdomains founded by DNScewl and Puredns"}' | notify --silent
rm permutations.txt
